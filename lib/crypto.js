...
const toBuffer = string => new Uint8Array(encode(atob(string)))
// uint8array -> base64 string
const toBase64 = array => btoa(decode(new Uint8Array(array)))
...
export const decrypt = async (cipherText, iv, key, alg = AES_GCM_OPTIONS) => {
    const buffer = await subtle.decrypt(
        Object.assign({ iv: encode(iv) }, alg),
        key,
        encode(cipherText)
    )
...
export const unwrapSecretKey = async credentials => {
    ...
    const { passphrase, alg: passphraseAlg } = await createPassphrase(hash, atob(salt), passphraseOptions, passwordAlg)
